...
flags.DEFINE_integer('patience', 100000, 'Patience for early stopping')
...
with tf.Graph().as_default():
  ...
  for step in range(1000):
      ...
      inff = sess.run(inf, feed_dict={
          x: vectors_validation,
          y: one_hot_labels_test,
          keep_prob: 1.0
      })
      #print inff[:,1:2]
      #print np.reshape(labels,(len(labels),1))
      print('Gini:',step+1, gini_tf(inff[:,1:2], np.reshape(labels_validation,(len(labels_validation),1))))
